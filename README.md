...
### 折线图 (Line Chart)
- 支持3D折线图渲染
- 支持四维数据格式 [Time, Y, X, Z]，其中Time为时间维度
- 向后兼容三维数据格式 [Y, X, Z]
- 支持时间动画播放
- 支持多种样式：基础、渐变、透明、点状
- 支持参考值高亮
...
#### 时间动画功能
- **时间轴支持**：第一维数据作为时间轴，支持时间序列数据可视化
- **动画播放**：可以播放、暂停、停止时间动画
- **循环播放**：支持循环播放时间序列动画
- **速度控制**：可调节动画播放速度
- **时间点控制**：可手动设置显示特定时间点的数据
...
- `SetCurrentTimePoint(float TimePoint)` - 设置当前显示的时间点
- `PlayTimeAnimation()` - 播放时间动画
- `PauseTimeAnimation()` - 暂停时间动画
- `StopTimeAnimation()` - 停止时间动画
- `GetAllTimePoints()` - 获取所有时间点
